...
|---|---|---|
| I. Evidence Over Assertion | `FindingStore.assign_verdict()` resolves every citation against a caller-supplied `resolver` before accepting `true-positive`; an unresolved citation force-demotes to `needs-review`, with the mismatch recorded in the report | `src/foundry/substrate/finding_store.py` |
| II. Surface Only What Survives | Not yet built — lands with the Detector/Reporter subagent definitions (notebooks 04+): Detector's `tools` list will have no path to a human-visible output, only `queue_candidate` | — |
...
| V. The Provider Is The Rate Arbiter | Not yet built — lands when an LLM provider is wired in (Indexer section onward): no internal concurrency cap below OpenAI's actual limit, rely on LangChain's retry/backoff | — |
