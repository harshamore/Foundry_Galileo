...
| V. The Provider Is The Rate Arbiter | The Indexer and Cartographer sections each make one real OpenAI call, but always a single request — neither exercises concurrency/backoff behavior. Not yet built: no internal concurrency cap below OpenAI's actual limit, rely on LangChain's retry/backoff. Lands meaningfully with the Detector's rule-sweep (many concurrent LLM calls, one per function) | — |
